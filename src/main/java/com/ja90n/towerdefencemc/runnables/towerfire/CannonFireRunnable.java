...
public class CannonFireRunnable extends BukkitRunnable {
    ...
    public CannonFireRunnable(TowerType type, Tower tower, TowerDefenceMC towerDefenceMC){
    ...
    public void run() {
        if (!armorStand.getNearbyEntities(range,range,range).isEmpty()) { // Checks if there are entities in range
            ...
            if (enemy != null) { // Checks if there is an entity that is an enemy
                ...
                for (Entity entity : enemy.getArmorStand().getNearbyEntities(1,1,1)){
                    if (entity instanceof ArmorStand){
                        Enemy target = enemyManager.getEnemy((ArmorStand) entity);
                        if (target != null){
                            target.damage(damage / 2);
                            target.getArmorStand().getWorld().spawnParticle(Particle.REDSTONE, enemy.getArmorStand().getLocation().add(0,0.5,0), 10, dustOptions);
                        }
                    }
                }
            }
